...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Start">
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'">
            <xsl:text>IEEE ZahDimaR</xsl:text>
          </xsl:when>
      ...
      <xsl:when test="b:category">
				<xsl:text>Похаченый мной</xsl:text>
			</xsl:when>
	...
	<xsl:template name="templ_str_OnlineCap" >
		...
		<xsl:value-of select="/*/b:Locals/b:Local[@LCID=INITIAL_CONTENTLCID]/b:Strings/b:OnlineCap"/>
			<xsl:text>Электронный ресурс</xsl:text>
	</xsl:template>
